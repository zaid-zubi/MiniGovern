...
async def submit_dataset_endpoint(
    ...
    current_user: Annotated[User, Depends(require_permission(Permission.DATASET_WRITE))],
    language: Annotated[Language, Query()] = Language.EN,
):
...
async def approve_dataset_endpoint(
    ...
    db: Annotated[AsyncSession, Depends(get_db)],
    background_tasks: BackgroundTasks,
    current_user: Annotated[User, Depends(require_permission(Permission.DATASET_APPROVE))],
    language: Annotated[Language, Query()] = Language.EN,
):
