...
from app.services.catagory import (
    assign_datasource_with_category,
    create_category,
    delete_category,
    get_category,
    unassign_datasource_with_category,
)
from app.services.catagory import update_category as update_category_service
...
async def update_category(
    ...
    return http_response(
        status=status.HTTP_201_CREATED,
        message=ResponseMessages.CATEGORY.UPDATED.get(language),
        data=data,
    )
...
async def remove_category(
    ...
    return http_response(
        ...
        message=ResponseMessages.CATEGORY.DELETED.get(language),
        data=data,
    )


@router.post("/{datasource_id}")
async def assign_datasource_to_category(
    datasource_id: int,
    category_id: int,
    db: Annotated[AsyncSession, Depends(get_db)],
    current_user: Annotated[User, Depends(require_permission(Permission.CATEGORY_WRITE))] = ...,
    ...
    return http_response(
        status=status.HTTP_200_OK, message=ResponseMessages.CATEGORY.ASSIGN.get(language), data=data
    )
...
async def unassign_category_from_datasource(
    ...
    return http_response(
        status=status.HTTP_200_OK,
        message=ResponseMessages.CATEGORY.UNASSIGN.get(language),
        data=data,
    )
